...
import { UploadedDocument } from '../lib/ts/interfaces';
...
export default function JobApplicationForm() {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<Schema['JobApplication']['type']>();

  // const coverLetterChecked = watch("coverLetter");

  const onSubmit: SubmitHandler<Schema['JobApplication']['type']> = async (
    data,
  ) => {
    console.log(data);
  ...
  const uploadFileToS3 = async (file: File) => {
    ...
    try {
      result = await uploadData({
        path: ({ identityId }) =>
          `files/${identityId}/resumes/${Date.now()}/${file.name}`,
        data: file,
        options: {
  ...
  const addDocumentField = () => {
    setDocuments([
      ...documents,
      { id: Date.now(), title: '', file: undefined, fileUrl: '' },
    ]);
  };
  ...
  return (
    <div className="form-container">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="form space-y-4">
            ...
            render={(messages) => {
              return (
                ...
                Object.entries(messages).map(([type, message]) => (
                  <p className="text-blue-500" key={type}>
                    {message?.toString()}
                  </p>
                ))
              );
        ...
        {watch('status') === 'interviewScheduled' && (
          <>
            <div>
              <label className="form-label" htmlFor="interviewType">
                Interview Type
              </label>
              <select
                className="select"
              ...
              >
                <option value="phone">Initial Phone Interview</option>
                <option value="hr">HR Screening</option>
                <option value="technical">Technical Interview</option>
                <option value="liveCoding">Live Coding Test</option>
        ...
        {documents.map((doc, index) => (
          <div
            key={doc.id}
            className="flex justify-items-center space-x-2 border border-gray-300 p-2 rounded-md mt-2"
          >
            {/* Document Title */}
            {/* <input
              type="text"
